...
/* Het document over pagina's leggen. Staat los van het bouwen van het bestand, zodat het scherm
   kan vragen hoeveel bladen het worden zonder er bytes voor te maken: het vel dat meekijkt is een
   natekening in HTML en weet dat zelf niet op de punt nauwkeurig. Hier valt de breuk echt. */
function leg(doc) {
  ...
  return { paginas, plaatjes };
}

export function maak(doc) {
  const { paginas, plaatjes } = leg(doc);
  return bouwBestand(paginas, plaatjes, doc);
}

/* Hoeveel bladen dit document wordt. */
export function telPaginas(doc) {
  return leg(doc).paginas.length;
}
