...
import { maakCalculator } from './calculator.js?v=d2afc15';
import { maakOfferte } from './offerte.js?v=d2afc15';
import { maakInmeetkaart } from './inmeten.js?v=d2afc15';
import { koppelAdresvelden } from './adres.js?v=d2afc15';
import * as PDF from './pdf.js?v=d2afc15';
import { offerteDocument } from './document.js?v=d2afc15';

const CONF = window.FRAMR_CONFIG;
const OAKLYN = CONF.OAKLYN_URL || 'https://oaklyn.nl';
...
const TABS = [
  { pad: '/', naam: 'Overzicht' },
  { pad: '/projecten', naam: 'Klussen', mod: 'projecten' },
...
/* De iconen in het menu: dunne lijntekeningen op een vast raster, een per onderdeel. */
const ICOON = (paden) => `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${paden}</svg>`;
const ICONEN = {
  '/': ICOON('<rect x="3" y="3" width="8" height="8" rx="1.5"/><rect x="13" y="3" width="8" height="8" rx="1.5"/><rect x="3" y="13" width="8" height="8" rx="1.5"/><rect x="13" y="13" width="8" height="8" rx="1.5"/>'),
  '/projecten': ICOON('<path d="M3 7h6l2 2h10v10H3z"/>'),
  '/inmeten': ICOON('<path d="M3 17l14-14 4 4-14 14H3z"/><path d="M8 12l2 2"/><path d="M11 9l2 2"/><path d="M14 6l2 2"/>'),
  '/calculaties': ICOON('<rect x="4" y="3" width="16" height="18" rx="2"/><path d="M8 8h8"/><path d="M8 12h3"/><path d="M13 12h3"/><path d="M8 16h3"/><path d="M13 16h3"/>'),
  '/offertes': ICOON('<path d="M6 3h9l4 4v14H6z"/><path d="M9 12h6"/><path d="M9 16h6"/>'),
  '/facturen': ICOON('<path d="M6 3h12v18l-3-2-3 2-3-2-3 2z"/><path d="M9 8h6"/><path d="M9 12h6"/>'),
  '/klanten': ICOON('<circle cx="12" cy="8" r="4"/><path d="M4 21c0-4 4-6 8-6s8 2 8 6"/>'),
  '/bestellingen': ICOON('<path d="M3 7l9-4 9 4-9 4z"/><path d="M3 7v10l9 4 9-4V7"/><path d="M12 11v10"/>'),
  '/bedrijf': ICOON('<path d="M4 10l8-6 8 6v10H4z"/><path d="M10 20v-6h4v6"/>'),
};
...
const MODULES = [
  ...
  { id: 'projecten', titel: 'Klussen', uitleg: 'De doos om een klus heen: inmeting, offertes en facturen bij elkaar. Een project is nooit verplicht.' },
];
...
const navBalk = document.querySelector('.fr-zij');
const zijvoet = document.getElementById('fr-zijvoet');
if (zijvoet) zijvoet.href = BASIS + '/bedrijf';
...
async function authVraag(pad, body) {
  ...
  if (!r.ok) {
    ...
    /* Eerst op de code en pas daarna op de zin. Supabase schrijft zijn meldingen soms om (de
       wachtwoordmelding noemt het aantal tekens, en dat aantal is een instelling), en dan valt een
       vertaling die op de letterlijke zin staat stil terug op Engels. De code verandert niet. */
    const OP_CODE = {
      weak_password: 'Kies een wachtwoord van minstens 8 tekens.',
      user_already_exists: 'Er bestaat al een account met dit e-mailadres. Log in, of vraag om vrijgave.',
      email_exists: 'Er bestaat al een account met dit e-mailadres. Log in, of vraag om vrijgave.',
      signup_disabled: 'Aanmelden staat op dit moment uit.',
      email_not_confirmed: 'Uw account is nog niet vrijgegeven. U krijgt bericht zodra u erin kunt.',
      invalid_credentials: 'Dit e-mailadres en wachtwoord horen niet bij elkaar. Nog geen account? Gebruik dan Account aanmaken.',
      validation_failed: 'Controleer het e-mailadres en het wachtwoord.',
      over_email_send_rate_limit: 'Er zijn net te veel berichten verstuurd. Probeer het over een minuut opnieuw.',
      over_request_rate_limit: 'Te veel pogingen achter elkaar. Probeer het over een minuut opnieuw.',
    };
    const NL = {
      'Email not confirmed': 'Uw account is nog niet vrijgegeven. U krijgt bericht zodra u erin kunt.',
      'Invalid login credentials': 'Dit e-mailadres en wachtwoord horen niet bij elkaar. Nog geen account? Gebruik dan Account aanmaken.',
      'Signups not allowed for this instance': 'Aanmelden staat op dit moment uit.',
      'User already registered': 'Er bestaat al een account met dit e-mailadres. Log in, of vraag om vrijgave.',
    };
    throw new Error(OP_CODE[inhoud.error_code] || NL[ruw] || ruw || 'Inloggen lukte niet.');
  }
  return inhoud;
}
...
/* De gegevens uit het aanmeldformulier reizen mee als user_metadata. De motor zet ze bij het eerste
   bezoek op de partnerrij, door een zeef heen (partners.ts, gegevensUitAanmelding), zodat een nieuw
   account niet leeg is en Jelle bij de vrijgave ziet wie er aanklopt. */
async function aanmelden(email, wachtwoord, gegevens = {}) {
  const t = await authVraag('/signup', { email, password: wachtwoord, data: gegevens });
  /* Staat bevestigen per mail uit, dan komt de sessie meteen mee. Die wordt hier bewaard en er gaat
     een enkel verzoek langs de motor, want daar gebeurt de rest: zijn partnerrij wordt aangemaakt
     met de gegevens uit dit formulier, en hij wordt bronsklant bij Oaklyn met een uitnodiging uit
     de winkel (keuze Jelle 13-09-2026). Zo staat dat er voordat hij het scherm sluit, en niet pas
     bij zijn eerste inlog. */
  if (t && t.access_token) {
    zetSessie({
      access_token: t.access_token,
      refresh_token: t.refresh_token,
      verloopt_op: Date.now() + (t.expires_in ?? 3600) * 1000,
      email: (t.user && t.user.email) || email,
    });
    await porMotor();
  }
}

/* Een enkel verzoek aan de motor, buiten motor() om. Dat laatste zou bij het antwoord 403 het
   wachtscherm tonen, en daar zijn we hier nog niet: de aanmelder hoort eerst te lezen dat het
   gelukt is. Wat er misgaat blijft hier, want dit is een duwtje en geen handeling van de
   gebruiker. */
async function porMotor() {
  const s = sessie();
  if (!s) return;
  try {
    await fetch(`${MOTOR}/mij`, { headers: { authorization: `Bearer ${s.access_token}` } });
  } catch { /* stil: de motor doet het bij zijn eerste inlog alsnog */ }
}
...
async function motor(pad, opties = {}) {
  ...
  const inhoud = await r.json().catch(() => ({}));
  /* Aangemeld maar nog niet vrijgegeven. Dat is geen fout van de gebruiker en hoort dus geen
     foutmelding te zijn, maar een scherm dat zegt waar hij op wacht. */
  if (r.status === 403 && inhoud.wacht_op_vrijgave) { toonWacht(inhoud.fout); throw new Error('wacht op vrijgave'); }
...
function kop(eyebrow, titel, onder, acties = '') {
  return `<header class="fr-paginakop"><div>
      ${eyebrow ? `<p class="fr-eyebrow">${schoon(eyebrow)}</p>` : ''}
      <h1 class="fr-kop">${schoon(titel)}</h1>
      ${onder ? `<p class="fr-onderkop">${schoon(onder)}</p>` : ''}
    </div>${acties ? `<div class="fr-paginakop-acties">${acties}</div>` : ''}</header>`;
}

/* De kolomkoppen boven een lijst met kolommen. De cellen dragen dezelfde klassen als die in een
   rij (hoofd, sub, maatje, sub) en dezelfde verdeling, zodat ze precies boven hun kolom staan. */
/* De overige acties van een rij, achter een knopje met drie puntjes. De naam van de rij is de weg
   naar binnen; wat je zelden doet (verwijderen, losmaken) hoort niet als knop in elke rij te staan. */
function meer(inhoud) {
  if (!String(inhoud ?? '').trim()) return '';
  return `<details class="fr-meer"><summary aria-label="Meer acties" title="Meer"><svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><circle cx="5" cy="12" r="1.8"/><circle cx="12" cy="12" r="1.8"/><circle cx="19" cy="12" r="1.8"/></svg></summary><div class="fr-meer-lijst">${inhoud}</div></details>`;
}

function kolomkop(koppen, klas = '', rechts = '') {
  const klassen = ['hoofd', 'sub', 'maatje', 'sub'];
  return `<div class="fr-th${klas ? ' ' + klas : ''}" aria-hidden="true">${koppen.map((k, i) => `<span class="${klassen[i] ?? 'sub'}">${schoon(k)}</span>`).join('')}<span class="rechts">${schoon(rechts)}</span></div>`;
}

/* Vier soorten stand, elk met een eigen stip: open (er valt iets te doen, oranje), wacht (het ligt
   bij een ander, een oranje ring), klaar (groen) en stil (grijs, voor wat concept, voorbij of weg is). */
function standklasse(stand) {
  ...
  if (['verstuurd', 'wacht_vloer', 'geoffreerd'].includes(stand)) return 'wacht';
  return 'open';
}
...
function stand(s, teksten = null) {
  ...
  const ruw = (teksten && teksten[sleutel]) || String(sleutel).replace(/_/g, ' ');
  const woord = ruw.charAt(0).toUpperCase() + ruw.slice(1);
  return `<span class="fr-stand ${standklasse(s)}"><i aria-hidden="true"></i>${schoon(woord)}</span>`;
}

/* De fase van een klus als maatlijn: zes streepjes, een per stap, tot waar de klus is oranje.
   Een verloren klus heeft een grijze lijn. */
const FASEN = ['open', 'wacht_vloer', 'geoffreerd', 'gewonnen', 'besteld', 'afgerond'];
function faseLijn(status) {
  const verloren = status === 'verloren';
  const tot = verloren ? 0 : Math.max(0, FASEN.indexOf(status || 'open'));
  return `<span class="fr-fase${verloren ? ' stil' : ''}" aria-hidden="true">${FASEN.map((f, i) => `<i class="${i <= tot ? 'aan' : ''}"></i>`).join('')}</span>`;
}

/* Een aantal met een icoontje ervoor, zoals de inmetingen en offertes in een klusrij. Nul komt er
   niet in: dat is ruis. */
function tel(icoon, n, een, veel) {
  if (!n) return '';
  return `<span class="fr-tel" title="${n} ${n === 1 ? een : veel}">${ICONEN[icoon] ?? ''}<b>${n}</b></span>`;
}

/* De winkelwagenlink van Oaklyn voor een offerte: uit de berekening erachter de regels met een
   variant in de winkel. Codes worden via de prijslijst aan een variant gekoppeld, want een
   berekening bewaart de code en niet de variant. */
async function oaklynMandjeVanOfferte(offerteId) {
  const { offerte } = await motor(`/offertes/${offerteId}`);
  if (!offerte?.calculation_id) return null;
  const [{ berekening }, prijzen] = await Promise.all([motor(`/berekeningen/${offerte.calculation_id}`), motor('/prijzen')]);
  const vidVan = new Map();
  (prijzen.vloeren ?? []).forEach((v) => { if (v.vid) vidVan.set(String(v.art), v.vid); });
  (prijzen.toebehoren ?? []).forEach((t) => { if (t.vid) vidVan.set(String(t.code), t.vid); });
  const mee = [];
  const niet = [];
  (berekening?.groepen ?? []).forEach((g) => (g.regels ?? []).forEach((r) => {
    if (r.soort !== 'materiaal' || r.bron !== 'framr' || !(Number(r.aantal) > 0)) return;
    const code = r.prijsbasis?.code ? String(r.prijsbasis.code) : '';
    const nummer = String(vidVan.get(code) ?? '').replace(/^gid:\/\/shopify\/ProductVariant\//, '');
    if (/^\d+$/.test(nummer)) mee.push(`${nummer}:${Math.max(1, Math.round(Number(r.aantal)))}`);
    else niet.push(r.omschrijving);
  }));
  if (!mee.length) return null;
  return { url: `${OAKLYN}/cart/${mee.join(',')}`, niet };
}
...
/* Het menu links staat uit zolang er niemand is ingelogd, en daarmee stond ook de naam uit: wie hier
   aankwam zag een formulier zonder merk. Deze regel zet hem terug op de pagina zelf. */
function merkregel() {
  return '<p class="fr-inlog__merk"><span class="naam">Framr<span class="one">.One</span></span><span class="label">Portaal</span></p>';
}


/* Aangemeld, nog niet vrijgegeven. Het account bestaat en het wachtwoord klopt; wat ontbreekt is
   het ja van Framr.One. Uitloggen kan, zodat iemand anders op dit apparaat wel verder kan. */
function toonWacht(melding) {
  navBalk.style.display = 'none';
  scherm.innerHTML = `
    <div class="fr-login">
      ${merkregel()}
      <p class="fr-eyebrow">Bijna</p>
      <h1 class="fr-kop">Uw account wacht op vrijgave.</h1>
      <p class="fr-onderkop">${schoon(melding || 'Uw aanmelding is binnen. U krijgt bericht zodra u erin kunt.')}</p>
      <div class="fr-knoppenrij">
        <button class="fr-knop tweede" type="button" id="fr-wacht-uit">Uitloggen</button>
      </div>
    </div>`;
  document.getElementById('fr-wacht-uit').addEventListener('click', uitloggen);
}
...
function toonLogin(melding) {
  ...
  scherm.innerHTML = `
    <div class="fr-inlog">
      ${merkregel()}
      <div class="fr-inlog__verhaal">
        <p class="fr-eyebrow">Voor professionals</p>
        <h1 class="fr-kop">Van maat tot factuur, en marge op het materiaal.</h1>
        <p class="fr-onderkop">Een gratis systeem dat u extra marge oplevert. U meet in,
        de offerte en de factuur rollen eruit, en het materiaal koopt u in op partnerprijs.</p>
        <ul class="fr-inlog__lijst">
          <li><b>Inmeten</b><span>Een kaart per ruimte, zo van de meter.</span></li>
          <li><b>Offerte</b><span>Uit de maten, met uw logo en uw nummers.</span></li>
          <li><b>Factuur</b><span>Uit de offerte, restbedragen bewaakt.</span></li>
          <li><b>Marge</b><span>Materiaal op partnerprijs, verschil voor u.</span></li>
        </ul>
      </div>
      <div class="fr-inlog__vak">
        <h2>Inloggen</h2>
        <p class="fr-onderkop">Met het e-mailadres van uw Framr.One-account.</p>
        ...
        <p class="fr-inlog__nieuw">Nog geen account?
          <button class="fr-actie" type="button" id="fr-aanmeld">Account aanmaken</button>
        </p>
      </div>
    </div>`;
  ...
  document.getElementById('fr-aanmeld').addEventListener('click', () => toonAanmelden(form.email.value.trim()));
}

/* Het aanmeldscherm. Een account aanmaken was een knop naast Inloggen die om een adres en een
   wachtwoord vroeg en verder niets; dat voelde niet als een account maken, en het leverde ook een
   lege partnerrij op. Hier vult iemand in een keer in wie hij is: het bedrijf, het KvK- en
   btw-nummer, hoe hij te bereiken is en waar hij zit.

   Die gegevens gaan mee als user_metadata en landen bij het eerste bezoek vanzelf op zijn rij. Hij
   hoeft ze dus niet nog eens in te vullen onder Mijn bedrijf, en Jelle ziet bij de vrijgave met wie
   hij te maken heeft in plaats van alleen een e-mailadres.

   Het postcodeveld zoekt de straat en de plaats erbij, hetzelfde hulpje als bij een klus.

   Het adresdeel van Uw bedrijf staat bewust op autocomplete uit. Met de gewone waarden (organization,
   street-address, postal-code) herkent Chrome het als een adresformulier en schuift hij zijn eigen
   lijst met opgeslagen adressen over het veld heen, ook over de bedrijfsnaam. Dat helpt hier
   niemand, want de postcode vult de straat en de plaats zelf al in, en het staat lelijk over het
   formulier. Bij het e-mailadres, het telefoonnummer en het wachtwoord blijft het aan; dat komt uit
   de wachtwoordbeheerder en is wel handig. Het KvK- en het btw-nummer staan er ook buiten, op
   verzoek van Jelle: daar heeft Chrome geen adreslijst voor, dus daar biedt hij hoogstens aan wat
   je zelf eerder in datzelfde veld hebt ingevuld, en dat is precies wel handig. */
function toonAanmelden(email = '') {
  navBalk.style.display = 'none';
  scherm.innerHTML = `
    <div class="fr-login fr-aanmeld">
      ${merkregel()}
      <p class="fr-eyebrow">Aanmelden</p>
      <h1 class="fr-kop">Maak uw account aan.</h1>
      <p class="fr-onderkop">U vult dit een keer in. Daarna staat het in uw account en komt het
      vanzelf op uw offertes en facturen te staan. Wij kijken de aanmelding na en geven hem vrij;
      u krijgt bericht zodra u erin kunt.</p>

      <form id="fr-aanmeldform" class="fr-form fr-form-breed">
        <p class="fr-blokkop"><span class="fr-eyebrow">Uw bedrijf</span></p>
        <div class="fr-veldrij">
          <label>Bedrijfsnaam<input name="bedrijfsnaam" required autocomplete="off"></label>
          <label>KvK-nummer<input name="kvk" required inputmode="numeric" placeholder="12345678"></label>
          <label>Btw-nummer<input name="btw_nummer" required placeholder="NL000000000B00"></label>
        </div>
        <div class="fr-veldrij">
          <label>Postcode<input name="postcode" autocomplete="off" placeholder="6431 AB"></label>
          <label>Adres<input name="adres" autocomplete="off"></label>
          <label>Plaats<input name="plaats" autocomplete="off"></label>
        </div>

        <p class="fr-blokkop"><span class="fr-eyebrow">Uw gegevens</span></p>
        <div class="fr-veldrij">
          <label>E-mailadres<input type="email" name="email" required autocomplete="email" value="${schoon(email)}"></label>
          <label>Telefoonnummer<input type="tel" name="telefoon" required autocomplete="tel" placeholder="06 12345678"></label>
          <label>Wachtwoord<input type="password" name="wachtwoord" required minlength="8" autocomplete="new-password" placeholder="minstens 8 tekens"></label>
        </div>

        <div class="fr-knoppenrij" style="margin-top:26px">
          <button class="fr-knop" type="submit">Account aanmaken</button>
          <button class="fr-knop tweede" type="button" id="fr-aanmeld-terug">Terug</button>
        </div>
        <p class="fr-formfout" id="fr-aanmeldfout"></p>
      </form>
    </div>`;

  const form = document.getElementById('fr-aanmeldform');
  const foutvak = document.getElementById('fr-aanmeldfout');
  koppelAdresvelden(form, { adres: 'adres', postcode: 'postcode', plaats: 'plaats' });
  document.getElementById('fr-aanmeld-terug').addEventListener('click', () => toonLogin());

  form.addEventListener('submit', eenmalig(form.querySelector('button[type=submit]'), async (e) => {
    e.preventDefault();
    foutvak.textContent = '';
    const lees = (naam) => String(form[naam]?.value ?? '').trim();
    try {
      await aanmelden(lees('email'), form.wachtwoord.value, {
        bedrijfsnaam: lees('bedrijfsnaam'),
        kvk: lees('kvk'),
        btw_nummer: lees('btw_nummer'),
        telefoon: lees('telefoon'),
        adres: lees('adres'),
        postcode: lees('postcode'),
        plaats: lees('plaats'),
      });
      /* Aanmelden en dan alsnog een scherm dat zegt dat je moet wachten is een drempel zonder doel:
         sinds 13-09-2026 wordt iedereen meteen toegelaten. Wie een sessie heeft gaat dus gewoon naar
         binnen. Alleen als die er niet is (bevestiging per mail aan) blijft er iets te lezen over. */
      if (sessie()) toon('/', true);
      else toonAangemeld(lees('email'));
    } catch (err) { foutvak.textContent = err.message; }
  }));
}

/* Na het aanmelden. Geen melding onder een formulier dat nog ingevuld staat, maar een eigen scherm:
   het is gelukt, en dit is wat er nu gebeurt. */
function toonAangemeld(email) {
  navBalk.style.display = 'none';
  scherm.innerHTML = `
    <div class="fr-login">
      ${merkregel()}
      <p class="fr-eyebrow">Gelukt</p>
      <h1 class="fr-kop">Uw aanmelding is binnen.</h1>
      <p class="fr-onderkop">We hebben uw gegevens ontvangen${email ? ` en het account staat op ${schoon(email)}` : ''}.
      U bent meteen aangemeld bij Oaklyn, onze groothandel, waar u op partnerprijs inkoopt; daar
      krijgt u een eigen uitnodiging voor. Bevestig uw e-mailadres via de mail die u zojuist hebt
      gekregen, dan kunt u hier inloggen.</p>
      <div class="fr-knoppenrij">
        <button class="fr-knop tweede" type="button" id="fr-aangemeld-terug">Naar inloggen</button>
      </div>
    </div>`;
  /* Uitloggen en niet alleen het scherm wisselen: bij het aanmelden is er een sessie gezet, en
     wie daarna op inloggen klikt hoort het inlogscherm te krijgen en niet het wachtscherm. */
  document.getElementById('fr-aangemeld-terug').addEventListener('click', uitloggen);
}
...
async function schermOverzicht() {
  ...
  /* Waar je begint staat in de kop: een klus of een inmeting. De rest zit in het menu. */
  const acties = [
    moduleAan('projecten') ? `<a class="fr-knop tweede" href="${BASIS}/projecten/nieuw" data-route>Nieuwe klus</a>` : '',
    moduleAan('inmeten') ? `<a class="fr-knop" href="${BASIS}/inmeten/nieuw" data-route>Nieuwe inmeting</a>` : '',
  ].join('');
  return kop('', 'Jouw werk, van maat tot factuur.', '', acties)
    ...
    + `<div class="fr-tegels">
        ...
        <div class="fr-tegel"><div class="naam">Te bestellen</div><div class="waarde${teBestellen.length ? ' let' : ''}">${teBestellen.length}</div></div>` : ''}
      ...
      ${moduleAan('projecten') ? `<div class="fr-blokkop"><h2>Lopende klussen</h2>
        <span class="rechts"><a class="fr-actie" href="${BASIS}/projecten" data-route>Alle klussen</a></span></div>
      ${kolomkop(['Klus', 'Klant en adres', 'Onderdelen'])}
      <div class="fr-rijen tabel">${projecten.slice(0, 5).map((x) => rijProject(x, klantnaam(x.customer_id))).join('') || legeRij('Nog geen klussen. Begin met een inmeting; de klus komt er vanzelf omheen.')}</div>` : ''}

      <div class="fr-twee">
        ${moduleAan('offertes') ? `<div>
          <div class="fr-blokkop"><h2>Laatste offertes</h2>
            <span class="rechts"><a class="fr-actie" href="${BASIS}/offertes" data-route>Alle offertes</a></span></div>
        ...
        ${moduleAan('inmeten') ? `<div>
          <div class="fr-blokkop"><h2>Laatste inmetingen</h2>
            <span class="rechts"><a class="fr-actie" href="${BASIS}/inmeten" data-route>Alle inmetingen</a></span></div>
...
const PROJECTSTANDEN = {
  open: 'open',
  wacht_vloer: 'wacht op vloerkeuze',
...
function rijProject(p, klantnaam = null) {
  ...
  const adres = [p.werkadres, p.plaats].filter(Boolean).join(', ');
  return `<div class="fr-rij">
      <span class="hoofd"><a href="${BASIS}/projecten/${p.id}" data-route>${schoon(p.naam)}</a></span>
      <span class="sub">${klantnaam ? `<span class="wie">${schoon(klantnaam)}</span>` : ''}${klantnaam && adres ? '<span class="punt"></span>' : ''}${adres ? `<span class="waar">${schoon(adres)}</span>` : ''}</span>
      <span class="maatje">${tel('/inmeten', metingen, 'inmeting', 'inmetingen')}${tel('/offertes', offertes, 'offerte', 'offertes')}</span>
      <span class="rechts">${faseLijn(p.status)}${stand(p.status, PROJECTSTANDEN)}
        ${meer(`<button class="fr-knop klein tweede" data-actie="project-weg" data-id="${p.id}">Klus verwijderen</button>`)}</span>
    </div>`;
}
...
async function schermProjecten() {
  ...
  return {
    html: kop('Klussen', 'De klus als doos.',
        'Klant, inmeting, berekening en offerte reizen samen. Verwijderen laat de inhoud staan.')
      + `<div class="fr-blokkop"><h2>Nieuwe klus</h2></div>
        ...
        <div class="fr-zoekbalk">
          <input id="pr-zoek" type="search" placeholder="Zoeken in je klussen" aria-label="Zoeken in je klussen" autocomplete="off" name="zoeken-klussen">
          <button type="button" class="fr-knop tweede klein" id="pr-filterknop" aria-expanded="false">${ICOON('<path d="M4 6h16"/><path d="M7 12h10"/><path d="M10 18h4"/>')}Filter</button>
        ...
        ${kolomkop(['Klus', 'Klant en adres', 'Onderdelen'])}
        <div class="fr-rijen tabel" id="pr-lijst"></div>
...
async function schermProject(id) {
  ...
  return {
    html: kop('Klus', project.naam,
      [klanten.find((k) => k.id === project.customer_id)?.naam,
        [project.werkadres, project.postcode, project.plaats].filter(Boolean).join(', ')].filter(Boolean).join(' · ') || 'nog geen klant of adres',
      `${faseLijn(project.status)}${stand(project.status, PROJECTSTANDEN)}${meer(`<button class="fr-knop klein tweede" data-actie="project-weg" data-id="${project.id}">Klus verwijderen</button>`)}`)
...
async function schermProjectNieuw(zoek) {
  ...
  return {
    html: kop('Klussen', 'Nieuwe klus', 'De klus is de doos waar de inmeting, de berekening en de offerte samen in reizen.')
      + `<form id="pr-form" class="fr-form fr-form-breed">
        ...
        <div class="fr-knoppenrij">
          <button class="fr-knop" type="submit">Aanmaken en meteen inmeten</button>
          <button class="fr-knop tweede" type="submit" id="pr-alleen">Alleen aanmaken</button>
          <a class="fr-knop tweede" href="${BASIS}/projecten" data-route>Terug</a>
        </div>
        <p class="fr-hint">Sta je bij de klant, dan ga je met de eerste knop rechtstreeks de inmeetkaart in;
          de klus en de klant reizen mee.</p>
        <p class="fr-formfout" id="pr-fout"></p>
      </form>`,
    na: () => {
      const form = document.getElementById('pr-form');
      koppelAdresvelden(form, { adres: 'werkadres', postcode: 'postcode', plaats: 'plaats' });
      /* Welke knop hij indrukt bepaalt waar hij daarna staat. Myron liep hier vast (testronde
         20-09-2026): na het maken van een klus moest hij links naar Inmeten en de klus daar
         opnieuw opzoeken, terwijl hij bij de klant staat en meteen wil meten. */
      let meteenInmeten = true;
      const alleen = document.getElementById('pr-alleen');
      if (alleen) alleen.addEventListener('click', () => { meteenInmeten = false; });
      form.addEventListener('submit', eenmalig(form.querySelector('button[type=submit]'), async (e) => {
        e.preventDefault();
        try {
          ...
          meldRegel('Klus aangemaakt.');
          const klantMee = project.customer_id ? `&klant=${project.customer_id}` : '';
          toon(meteenInmeten ? `/inmeten/nieuw?project=${project.id}${klantMee}` : `/projecten/${project.id}`, true);
...
/* De inmetingrij. Op de eigen lijst dragen de vier knoppen de hele vervolgweg; in de smalle kolom
   ...
   weg te kunnen door een misklik in een doos. Verwijderen doet hij onder Inmeten, waar de inmeting zelf
   woont. Dezelfde regel als bij de klant van een klus, die daar ook alleen los te halen is. */
/* De inmetingrij. Op de eigen lijst staat de klus erbij en in de klus zelf niet, want daar weet je
   al waar je bent. Myron miste dat in zijn testronde (20-09-2026): met meerdere klussen zie je een
   lijst met ruimtes en niet waar ze bij horen, en dan is niet te achterhalen waar je mee bezig
   bent. */
function rijInmeting(x, kort = false, inKlus = null, klusNaam = null) {
  const ruimtes = (x.portal_measurement_rooms ?? []).length;
  return `<div class="fr-rij">
      <span class="hoofd"><a href="${BASIS}/inmeten/${x.id}" data-route>${schoon(x.naam || 'Inmeting')}</a></span>
      <span class="sub">${klusNaam
        ? `<span class="wie">${schoon(klusNaam)}</span><span class="punt"></span><span class="waar">${datum(x.gemeten_op)}</span>`
        : datum(x.gemeten_op)}</span>
      <span class="maatje">${ruimtes ? `<span class="tal">${ruimtes} ruimte${ruimtes === 1 ? '' : 's'}</span><span class="geld">${m2(x.totaal_m2)}</span>` : ''}${(x.kozijnen ?? []).length ? (() => {
          const stuks = x.kozijnen.reduce((s, kz) => s + (Number(kz.aantal) || 1), 0);
          return `<span class="wanneer">${stuks} kozijn${stuks === 1 ? '' : 'en'}</span>`;
        })() : ''}${ruimtes || (x.kozijnen ?? []).length ? '' : 'leeg'}</span>
      <span class="rechts">
        ${kort ? '' : `<a class="fr-actie" href="${BASIS}/calculaties/nieuw?van=${x.id}" data-route>Doorrekenen</a>`}
        ${kort ? '' : meer(`<a class="fr-knop klein tweede" href="${BASIS}/offertes/nieuw?inmeting=${x.id}" data-route>Offerte maken</a>
          ${inKlus
            ? `<button class="fr-knop klein tweede" data-actie="inmeting-los" data-id="${x.id}">Uit deze klus halen</button>`
            : `<button class="fr-knop klein tweede" data-actie="inmeting-weg" data-id="${x.id}">Inmeting verwijderen</button>`}`)}
      </span>
    </div>`;
}
...
function rijBerekening(b, projecten = null) {
  ...
  const klusKeuze = projecten
    ...
    : '';
  return `<div class="fr-rij">
      <span class="hoofd"><a href="${BASIS}/calculaties/${b.id}" data-route>${schoon(b.naam || 'Berekening')}</a></span>
      <span class="sub">${datum(b.created_at)}</span>
      <span class="maatje"><span class="tal">${m2(b.oppervlak_m2)}</span><span class="geld">${euro(b.totaal_verkoop)}</span></span>
      <span class="rechts">${klusKeuze}
        <a class="fr-actie" href="${BASIS}/offertes/nieuw?berekening=${b.id}" data-route>Naar offerte</a>
      </span>
    </div>`;
}
...
function rijFactuur(f) {
  return `<div class="fr-rij">
      <span class="hoofd"><a href="${BASIS}/facturen/${f.id}" data-route>${schoon(f.nummer)}</a></span>
      <span class="sub">${schoon(f.klant_naam || '')}</span>
      <span class="maatje"><span class="geld">${euro(f.totaal)}</span><span class="wanneer">vervalt ${datum(f.vervaldatum)}</span></span>
      <span class="rechts">${stand(f.status)}
        ${f.status === 'verstuurd' ? `<button class="fr-actie" data-actie="factuur-status" data-id="${f.id}" data-status="betaald">Betaald</button>` : ''}
        ${f.status === 'concept' ? `<button class="fr-actie" data-actie="factuur-status" data-id="${f.id}" data-status="verstuurd">Verstuurd</button>` : ''}
        ${/* Een misklik op Betaald was hier niet terug te draaien: de knop verdween en de weg terug
              stond alleen in de factuur zelf, in een keuzelijst die niemand daarvoor opent. Gemeld
              door Myron, 20-09-2026. */ ''}
        ${f.status === 'betaald' ? `<button class="fr-actie" data-actie="factuur-status" data-id="${f.id}" data-status="verstuurd">Toch niet betaald</button>` : ''}
      </span>
    </div>`;
}

async function schermInmeten() {
  /* De klussen komen erbij om de naam te kunnen tonen; de inmeting draagt alleen het id. */
  const [{ inmetingen }, { projecten }] = await Promise.all([motor('/inmetingen'), motor('/projecten')]);
  const klusVan = new Map((projecten ?? []).map((p) => [p.id, p.naam]));
  return kop('Inmeten', 'Per ruimte, op je telefoon, op locatie.',
    ...
    + `<div class="fr-blokkop"><h2>Inmetingen</h2><span class="maatje">${inmetingen.length}</span>
      ...
      ${kolomkop(['Inmeting', 'Klus en datum', 'Ruimtes en maat'])}
      <div class="fr-rijen tabel">${inmetingen.map((x) => rijInmeting(x, false, null, klusVan.get(x.project_id) || 'geen klus')).join('') || legeRij('Nog geen inmetingen. Druk op Nieuwe inmeting om te beginnen.')}</div>`;
}
...
function schermInmetingForm(id, zoek) {
  ...
  return {
    /* Bewaren staat ook in de kop: de knop hoort bij het formulier via zijn form-attribuut, zodat
       hij van bovenaan hetzelfde doet als de knop in de bewaarbalk onderaan. */
    html: kop('Inmeten', id === 'nieuw' ? 'Nieuwe inmeting' : 'Inmeting',
      ...
      `<a class="fr-knop tweede" href="${BASIS}/inmeten" data-route>Terug</a><button class="fr-knop" type="submit" form="im-form">Bewaren</button>`)
  ...
  return {
    html: kop('Calculaties', 'Van maat naar materiaallijst.',
      ...
      + `<div class="fr-blokkop"><h2>Berekeningen</h2><span class="maatje">${berekeningen.length}</span>
        ...
        ${kolomkop(['Berekening', 'Gemaakt op', 'Oppervlak en verkoop'])}
        <div class="fr-rijen tabel" id="ca-lijst">${berekeningen.map((b) => rijBerekening(b, projecten ?? [])).join('') || legeRij('Nog geen bewaarde berekeningen.')}</div>`,
    na: () => {
...
async function schermCalculatorNieuw(zoek) {
  ...
  return {
    html: kop('Calculatie', 'Reken het project door.', onder, `<a class="fr-knop tweede" href="${BASIS}/calculaties" data-route>Terug</a>`)
...
async function schermBerekening(id) {
  ...
  if (!b) {
    return kop('Calculatie', 'Deze berekening bestaat niet meer.', null)
      + `<div class="fr-leeg"><div class="naam">Niet gevonden</div>
        Hij is verwijderd, of hij hoort bij een ander account.</div>
  ...
  return {
    ...
    <div class="fr-knoppenrij">
      ...
      <button class="fr-knop tweede" data-actie="berekening-weg" data-id="${b.id}">Verwijderen</button>
...
function rijOfferte(o) {
  return `<div class="fr-rij">
      <span class="hoofd"><a href="${BASIS}/offertes/${o.id}" data-route>${schoon(o.nummer)}</a></span>
      <span class="sub">${schoon(o.klant_naam || '')}</span>
      <span class="maatje"><span class="geld">${euro(o.totaal)}</span><span class="wanneer">${datum(o.created_at)}</span></span>
      <span class="rechts">${stand(o.status)}</span>
    </div>`;
}
...
async function schermOffertes() {
  ...
  return kop('Offertes', 'Op jouw naam, met jouw marge.',
    ...
    + `<div class="fr-tegels">
        ...
        <div class="fr-tegel"><div class="naam">Verdiend</div><div class="waarde geld">${euro(verdiend)}</div></div>
      </div>
      ...
      ${kolomkop(['Offerte', 'Klant', 'Bedrag en datum'])}
      <div class="fr-rijen tabel">${offertes.map((o) => rijOfferte(o)).join('') || legeRij('Nog geen offertes.')}</div>`;
}
...
/* Wat elke stand betekent. Myron vroeg in zijn testronde (20-09-2026) wat gewonnen en verloren
   nu eigenlijk zeggen; de woorden staan in een keuzelijst zonder enige uitleg, terwijl gewonnen
   wel degelijk iets in gang zet (dan verschijnt Bestellen bij Oaklyn). */
const STANDUITLEG = {
  concept: 'Concept: je bent er nog mee bezig, hij is niet de deur uit en je kunt alles nog wijzigen.',
  verstuurd: 'Verstuurd: hij ligt bij je klant en je wacht op antwoord. Wat erop staat verandert niet meer.',
  gewonnen: 'Gewonnen: je klant heeft ja gezegd. Hier bestel je het materiaal en maak je de aanbetalingsfactuur.',
  verloren: 'Verloren: het is niet doorgegaan. De offerte blijft staan om later nog eens te bekijken of te kopieren.',
  besteld: 'Besteld: het materiaal is bij Oaklyn besteld. Alleen de stand verandert, de offerte blijft zoals hij was.',
};
...
async function schermOfferte(id) {
  ...
  /* Wat je met een offerte doet staat in de kop: bestellen als hij gewonnen is, anders de factuur,
     bewerken zolang hij concept is, en de rest achter de puntjes. */
  const acties = [
    ...
    offerte.status === 'gewonnen'
      ? `<button class="fr-knop" data-actie="offerte-bestel" data-id="${offerte.id}">Bestellen bij Oaklyn</button>
         <a class="fr-knop tweede" href="${BASIS}/offertes/${offerte.id}/factuur" data-route>Factuur maken</a>`
      : `<a class="fr-knop" href="${BASIS}/offertes/${offerte.id}/factuur" data-route>Factuur maken</a>`,
    meer(`<button class="fr-knop klein tweede" data-actie="offerte-pdf" data-id="${offerte.id}">PDF opslaan</button>
      <button class="fr-knop klein tweede" data-actie="offerte-kopie" data-id="${offerte.id}">Kopieren als nieuwe offerte</button>
      <button class="fr-knop klein tweede" data-actie="offerte-weg" data-id="${offerte.id}">Offerte verwijderen</button>`),
  ].join('');
  return {
    html: kop('Offerte', offerte.nummer,
      `${waar} · gemaakt ${datum(offerte.created_at)}${offerte.geldig_tot ? ` · geldig tot ${datum(offerte.geldig_tot)}` : ''}`,
      `${stand(offerte.status)}${acties}`)
    ...
    <div class="fr-veldrij" style="max-width:620px">
      <label class="fr-veldlabel">Stand
        ...
        <span class="fr-hint" id="of-standuitleg">${schoon(STANDUITLEG[offerte.status] ?? '')}</span>
      </label>
    ...
    ${kolomkop(['Regel', 'Toelichting', 'Aantal'], 'regels', 'Bedrag')}
    <div class="fr-rijen tabel regels">${regels.map((r) => `<div class="fr-rij">
        <span class="hoofd">${schoon(r.omschrijving ?? 'Regel')}</span>
        <span class="sub">${schoon(r.uitleg ?? '')}</span>
        <span class="maatje">${r.aantal ?? ''} ${schoon(r.eenheid ?? '')}</span>
        <span class="rechts"><span class="geld">${euro(r.bedrag_verkoop ?? 0)}</span></span>
    ...
    <div class="fr-tegels">
      <div class="fr-tegel"><div class="naam">Totaal</div><div class="waarde geld">${euro(offerte.totaal)}</div></div>
      <div class="fr-tegel"><div class="naam">Jouw marge</div><div class="waarde geld">${euro(marge(offerte))}</div></div>
    ...
    na: () => {
      document.getElementById('of-stand').addEventListener('change', async (e) => {
        const uitleg = document.getElementById('of-standuitleg');
        if (uitleg) uitleg.textContent = STANDUITLEG[e.target.value] ?? '';
        try {
...
function rijDocument(d) {
  return `<div class="fr-rij">
      ...
      <span class="rechts">
        <button class="fr-actie" data-actie="document-halen" data-id="${d.id}">Ophalen</button>
        ${meer(`<button class="fr-knop klein tweede" data-actie="document-weg" data-id="${d.id}">Uit het archief halen</button>`)}
      </span>
    </div>`;
}
...
async function schermFacturen() {
  ...
  return kop('Facturen', 'Uit de offerte of helemaal los, in jouw reeks.',
    ...
    + `<div class="fr-blokkop"><h2>Alle facturen</h2><span class="maatje">${facturen.length}</span>
      ...
      ${kolomkop(['Factuur', 'Klant', 'Bedrag en vervaldatum'])}
      <div class="fr-rijen tabel">${facturen.map((f) => rijFactuur(f)).join('') || legeRij('Nog geen facturen.')}</div>
...
async function schermOfferteBewerken(id) {
  ...
  const invoer = {
    ...
    /* Het nummer reist mee zodat het vel dat meekijkt het echte nummer draagt en niet leeg blijft;
       bij een nieuwe offerte is er nog geen nummer, dat geeft de motor pas bij het bewaren. */
...
async function schermFactuur(id) {
  ...
  /* De PDF is wat je met een factuur doet; de rest staat achter de puntjes. */
  ...
  const acties = [
    ...
    meer(`<button class="fr-knop klein tweede" data-actie="factuur-csv" data-id="${factuur.id}">CSV voor de boekhouding</button>
      ${factuur.quote_id ? `<a class="fr-knop klein tweede" href="${BASIS}/offertes/${factuur.quote_id}" data-route>Naar de offerte</a>` : ''}
      ${factuur.customer_id ? `<a class="fr-knop klein tweede" href="${BASIS}/klanten/${factuur.customer_id}" data-route>Naar de klant</a>` : ''}
      ${factuur.status === 'concept' ? `<button class="fr-knop klein tweede" data-actie="factuur-weg" data-id="${factuur.id}">Concept verwijderen</button>` : ''}`),
  ].join('');
  return {
    html: kop('Factuur', factuur.nummer,
      `${waar} · ${datum(factuur.factuurdatum)}${factuur.vervaldatum ? ` · vervalt ${datum(factuur.vervaldatum)}` : ''}${teLaat ? ' · over de vervaldatum' : ''}${adresregel ? ` · ${adresregel}` : ''}`,
      `${stand(factuur.status)}${acties}`)
...
async function schermKlanten() {
  ...
  return kop('Klanten', 'Jouw klanten, aan jouw klussen geknoopt.',
    ...
    + `<div class="fr-blokkop"><h2>Alle klanten</h2><span class="maatje">${klanten.length}</span>
      ...
      ${kolomkop(['Klant', 'Contact en plaats', 'Telefoon en e-mail', 'Klussen'], 'klanten')}
      <div class="fr-rijen tabel klanten">${klanten.map((k) => {
        ...
        return `<div class="fr-rij">
          <span class="hoofd"><a href="${BASIS}/klanten/${k.id}" data-route>${schoon(k.naam)}</a></span>
...
/* Het klantendetail: zijn kaart om bij te werken, met daaronder het koppelweb, alles wat
   op nummer aan deze klant hangt. Verwijderen laat dat allemaal staan; alleen de kaart verdwijnt. */
async function schermKlant(id) {
  ...
  return {
    html: kop('Klant', klant.naam,
      [klant.contact, [klant.telefoon, klant.email].filter(Boolean).join(' · '), [klant.postcode, klant.plaats].filter(Boolean).join(' ')].filter(Boolean).join(' · '),
      `<a class="fr-knop" href="${BASIS}/projecten/nieuw?klant=${klant.id}" data-route>Nieuwe klus</a>`)
    + `<form id="kl-form" class="fr-form fr-form-breed">
        ...
        <div class="fr-knoppenrij">
          <button class="fr-knop" type="submit">Bewaren</button>
          <button class="fr-knop tweede" type="button" data-actie="klant-weg" data-id="${klant.id}">Klant verwijderen</button>
          <a class="fr-knop tweede" href="${BASIS}/klanten" data-route>Terug</a>
        </div>
        <p class="fr-formfout" id="kl-fout"></p>
      </form>
      <div class="fr-blokkop"><h2>Klussen van deze klant</h2><span class="maatje">${projecten.length}</span></div>
...
async function schermBestellingen() {
  ...
  return kop('Bestellingen', 'Bestellen loopt via Oaklyn.',
      'Vanuit een berekening of een gewonnen offerte vul je met een knop de winkelwagen van de webshop; daar reken je af op je partnerprijs. Losse artikelen bestel je in de webshop zelf.',
      `<a class="fr-knop" href="${OAKLYN}" target="_blank" rel="noopener">Naar de webshop van Oaklyn</a>`)
    + `<div class="fr-blokkop"><h2>Alle bestellingen</h2><span class="maatje">${bestellingen.length}</span></div>
      ...
      <div class="fr-blokkop"><h2>Wat er nog moet gebeuren voor de bestelknop</h2></div>
      <div class="fr-rijen">
      ...
      <p class="fr-hint">Tot die tijd gaat de laatste stap via de winkelwagen van Oaklyn: de knop Bestellen bij Oaklyn in de
        calculator en op een gewonnen offerte zet de materiaallijst daar klaar. Je offerte, je calculatie en je
        facturen werken gewoon.</p>`;
}
...
async function schermBedrijf() {
  ...
  zetBedrijfsnaam(partner.bedrijfsnaam);
  tekenTabs('/bedrijf');
  const veld = (naam, label, type = 'text') =>
    ...
    + `<div class="fr-blokkop"><h2>Wat gebruik je</h2></div>
      ...
      <div class="fr-logoveld">
        ...
        <div>
          ...
          <button class="fr-knop klein tweede" type="button" id="fr-logo-weg" ${partner.logo_data ? '' : 'hidden'}>Logo verwijderen</button>
...
async function offertePdf(id) {
  ...
  /* Wat er op het vel staat komt uit document.js, dezelfde beschrijving die het scherm tekent.
     Zo kan de PDF nooit iets anders zeggen dan wat hij tijdens het invullen zag. */
  const bytes = PDF.maak(offerteDocument({
    partner,
    logo: beeld,
    kopregels: partnerKopregels(partner),
    klant: klantVel,
    nummer: offerte.nummer,
    datum: datum(offerte.created_at),
    geldigTot: offerte.geldig_tot ? datum(offerte.geldig_tot) : '',
    levertijd: offerte.levertijd || '',
    project: offerte.project || (offerte.berekening && offerte.berekening.vloer) || '',
    materiaal,
    werk,
    matAdvies: mat.advies,
    matVerkoop: mat.verkoop,
    werkTotaal: Number(offerte.werk_totaal ?? 0),
    subtotaal: Number(offerte.subtotaal ?? 0),
    btwBedrag: Number(offerte.btw_bedrag ?? 0),
    totaal: Number(offerte.totaal ?? 0),
    btwPct,
    euro,
  }));
...
async function actie(knop) {
  ...
  try {
    ...
    if (wat === 'project-weg') {
      if (!confirm('Deze klus verwijderen? De inmetingen en offertes blijven bestaan.')) return;
      await motor(`/projecten/${id}`, { methode: 'DELETE' });
      meldRegel('Klus verwijderd.');
      toon('/projecten', true);
      return;
    }
    /* Uit de klus halen, niet weggooien: de inmeting blijft bestaan en staat daarna los onder
       Inmeten. Verwijderen kan alleen daar, waar de inmeting zelf woont. */
    if (wat === 'inmeting-los') {
    ...
    if (wat === 'inmeting-weg') {
      if (!confirm('Deze inmeting definitief verwijderen, met haar ruimtes?')) return;
      await motor(`/inmetingen/${id}`, { methode: 'DELETE' });
      meldRegel('Inmeting verwijderd.');
      toon(huidigeRoute(), false);
      return;
    }
    if (wat === 'berekening-weg') {
      if (!confirm('Deze berekening verwijderen?')) return;
    ...
    if (wat === 'offerte-bestel') {
      /* Tot de besteladapter er is gaat bestellen via de winkelwagen van Oaklyn: de materiaalregels
         van de berekening achter de offerte, elk met zijn variant en aantal, als winkelwagenlink.
         Daar logt hij in en rekent hij af op zijn partnerprijs. Zonder berekening erachter (een
         vrije offerte) gaat hij naar de webshop zelf. */
      const link = await oaklynMandjeVanOfferte(id);
      if (!link) { meldRegel('Deze offerte heeft geen berekening met winkelartikelen achter zich; de webshop gaat open.'); window.open(OAKLYN, '_blank', 'noopener'); return; }
      if (link.niet.length) meldRegel(`Niet in de winkel gevonden en dus niet meegegaan: ${link.niet.join(', ')}.`);
      window.open(link.url, '_blank', 'noopener');
      return;
    }
    if (wat === 'offerte-weg') {
      if (!confirm('Deze offerte verwijderen? Facturen blijven staan.')) return;
    ...
    if (wat === 'document-weg') {
      if (!confirm('Dit bestand uit het archief verwijderen? Het vel zoals het de deur uit ging is dan weg.')) return;
    ...
    if (wat === 'factuur-weg') {
      if (!confirm('Dit concept verwijderen? Een verstuurde factuur kan nooit weg.')) return;
      await motor(`/facturen/${id}`, { methode: 'DELETE' });
      meldRegel('Concept verwijderd.');
      /* Met duw, anders blijft het adres op de verwijderde factuur staan en loop je bij verversen
    ...
    if (wat === 'klant-weg') {
      if (!confirm('Deze klant verwijderen? De klussen, offertes en facturen blijven staan; alleen de kaart verdwijnt.')) return;
      await motor(`/klanten/${id}`, { methode: 'DELETE' });
      meldRegel('Klant verwijderd.');
      toon('/klanten', true);
      return;
    }
  } catch (e) {
...
function tekenTabs(actief) {
  tabbalk.innerHTML = TABS
    ...
    .map((t) => `<a class="fr-tab${t.pad === actief ? ' actief' : ''}${t.pad === '/bedrijf' ? ' bedrijf' : ''}" href="${BASIS}${t.pad}" data-route><span class="tick"></span>${ICONEN[t.pad] ?? ''}<span>${t.naam}</span></a>`)
    .join('');
  if (zijvoet) zijvoet.classList.toggle('actief', actief === '/bedrijf');
}

/* De voet van het menu draagt de naam van zijn bedrijf, zodra de motor die verteld heeft. */
function zetBedrijfsnaam(naam) {
  const el = zijvoet?.querySelector('.naam');
  if (el) el.textContent = (naam || '').trim() || 'Mijn bedrijf';
}
  ...
  if (bedrijfform) {
    ...
    bedrijfform.addEventListener('submit', eenmalig(bedrijfform.querySelector('button[type=submit]'), async (e) => {
      ...
      try {
        ...
        zetBedrijfsnaam(body.bedrijfsnaam);
...
document.addEventListener('click', (e) => {
  document.querySelectorAll('details.fr-meer[open]').forEach((d) => { if (!d.contains(e.target)) d.removeAttribute('open'); });
...
/* De maatlijn in het menu: de basislijn met een streepje per onderdeel staat in de schil, de
   oranje voortgang tekent mee met het scrollen van het blad. */
...
function tekenVoortgang() {
  ...
  voortgang.style.height = (deel * 100) + '%';
}

window.addEventListener('scroll', tekenVoortgang, { passive: true });
window.addEventListener('resize', tekenVoortgang);
