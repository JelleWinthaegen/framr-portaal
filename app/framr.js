...
import { maakCalculator } from './calculator.js?v=8d33d6c';
import { maakOfferte } from './offerte.js?v=8d33d6c';
import { maakInmeetkaart } from './inmeten.js?v=8d33d6c';
import { koppelAdresvelden } from './adres.js?v=8d33d6c';
import * as PDF from './pdf.js?v=8d33d6c';
...
/* De overige acties van een rij, achter een knopje met drie puntjes. De naam van de rij is de weg
   naar binnen; wat je zelden doet (weghalen, losmaken) hoort niet als knop in elke rij te staan. */
...
function rijProject(p, klantnaam = null) {
  ...
  return `<div class="fr-rij">
      ...
      <span class="rechts">${faseLijn(p.status)}${stand(p.status, PROJECTSTANDEN)}
        ${meer(`<button class="fr-knop klein tweede" data-actie="project-weg" data-id="${p.id}">Klus weghalen</button>`)}</span>
    </div>`;
}
...
async function schermProject(id) {
  ...
  return {
    html: kop('Klus', project.naam,
      ...
      `${faseLijn(project.status)}${stand(project.status, PROJECTSTANDEN)}${meer(`<button class="fr-knop klein tweede" data-actie="project-weg" data-id="${project.id}">Klus weghalen</button>`)}`)
      ...
      <span class="rechts">
        ...
        ${kort ? '' : meer(`<a class="fr-knop klein tweede" href="${BASIS}/offertes/nieuw?inmeting=${x.id}" data-route>Offerte maken</a>
          ${inKlus
            ...
            : `<button class="fr-knop klein tweede" data-actie="inmeting-weg" data-id="${x.id}">Inmeting weghalen</button>`}`)}
      </span>
    </div>`;
}
...
async function schermInmeten() {
  ...
  return kop('Inmeten', 'Per ruimte, op je telefoon, op locatie.',
    ...
    + `<div class="fr-blokkop"><h2>Inmetingen</h2><span class="maatje">${inmetingen.length}</span>
      ...
      ${kolomkop(['Inmeting', 'Gemeten op', 'Ruimtes en maat'])}
      <div class="fr-rijen tabel">${inmetingen.map((x) => rijInmeting(x)).join('') || legeRij('Nog geen inmetingen. Druk op Nieuwe inmeting om te beginnen.')}</div>`;
}
...
async function schermOfferte(id) {
  ...
  const acties = [
    ...
    meer(`<button class="fr-knop klein tweede" data-actie="offerte-pdf" data-id="${offerte.id}">PDF opslaan</button>
      ...
      <button class="fr-knop klein tweede" data-actie="offerte-weg" data-id="${offerte.id}">Offerte weghalen</button>`),
  ].join('');
  return {
...
async function schermFactuur(id) {
  ...
  const acties = [
    ...
    meer(`<button class="fr-knop klein tweede" data-actie="factuur-csv" data-id="${factuur.id}">CSV voor de boekhouding</button>
      ...
      ${factuur.status === 'concept' ? `<button class="fr-knop klein tweede" data-actie="factuur-weg" data-id="${factuur.id}">Concept weghalen</button>` : ''}`),
  ].join('');
  return {
