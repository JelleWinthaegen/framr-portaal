...
import { velHtml, pasSchaal } from './vel.js?v=d2afc15';
import * as PDF from './pdf.js?v=d2afc15';

/* Het vel schaalt mee met het venster. De luisteraar hangt een keer aan de pagina en doet niets
   zodra er geen vel meer op het scherm staat, want schermen worden hier vervangen en niet
   opgeruimd. */
let venstervolger = false;
function volgVenster() {
  if (venstervolger) return;
  venstervolger = true;
  let wacht = null;
  window.addEventListener('resize', () => {
    clearTimeout(wacht);
    wacht = setTimeout(() => {
      const doos = document.getElementById('of-vel');
      if (doos) pasSchaal(doos);
    }, 80);
  });
}
...
export function maakOfferte({ motor, euro, schoon, meldRegel, naBewaren }) {
  ...
  let bron = null;
  let bewerktId = null;
  /* Het logo als beeld voor de paginatelling: het neemt ruimte in op het blad, dus zonder dit zou
     de telling een offerte met logo te laag inschatten. Wordt een keer geladen bij het tonen. */
  let logoBeeld = null;
  ...
  function tekenTotalen() {
    ...
    const kortingUit = $('of-kortinguit');
    if (kortingUit) {
      const winst = t.matVerk - t.matInk;
      kortingUit.textContent = geenNiveau
        ? `Je klant betaalt ${euro(t.matVerk)} voor het materiaal.`
        : `Je klant betaalt ${euro(t.matVerk)} voor het materiaal, jij houdt daar ${euro(winst)} aan over.`;
    }
    /* Alles wat de som verandert, verandert ook het vel. Daarom hangt het vel hieraan en niet aan
       elk veld apart: een nieuw invoerveld komt dan vanzelf op het vel terecht. */
    tekenVel();
  }

  /* Het vel dat meekijkt.

     Dezelfde beschrijving als de PDF (document.js), gevuld met wat er op dit moment in de velden
     staat. Daarom is wat hij ziet altijd wat de klant krijgt: er is geen tweede verhaal over hoe
     een offerte eruitziet. Wens van Jelle, 17-09-2026.

     De klantgegevens komen uit de velden en niet uit de kaart: hij mag ze hier overschrijven voor
     een losse klant, en dan hoort het vel die te tonen. */
  function documentNu() {
    const t = totalen();
    const veld = (id) => { const el = $(id); return el ? el.value.trim() : ''; };
    const factor = 1 - korting / 100;
    ...
    const materiaal = [
      ...
      ...vrijMat.map((r) => {
        const bedrag = num(r.aantal) * num(r.prijs);
        return {
          omschrijving: r.omschrijving || '',
          aantal: r.aantal || '',
          eenheid: r.eenheid || '',
          bedrag_advies: bedrag,
          bedrag_verkoop: bedrag,
        };
      }),
    ].filter((r) => r.omschrijving || r.bedrag_verkoop);

    const werkregels = werk
      .map((w) => ({
      ...
      .filter((w) => w.omschrijving || w.bedrag_verkoop);

    const klus = projecten.find((p) => p.id === veld('of-project'));
    const dagen = Number(veld('of-geldig')) || 0;
    const tot = new Date();
    tot.setDate(tot.getDate() + dagen);
    const nlDatum = (d) => `${String(d.getDate()).padStart(2, '0')}-${String(d.getMonth() + 1).padStart(2, '0')}-${d.getFullYear()}`;

    return offerteDocument({
      partner,
      logo: logoBeeld,
      logoUrl: partner.logo_data || null,
      kopregels: [
      ...
      klant: {
        naam: veld('of-klant-naam'),
        adres: veld('of-klant-adres'),
        postcode: veld('of-klant-postcode'),
        plaats: veld('of-klant-plaats'),
      },
      nummer: bron.nummer || '',
      datum: nlDatum(new Date()),
      geldigTot: dagen > 0 ? nlDatum(tot) : '',
      project: (klus && klus.naam) || bron.vloer || '',
      materiaal,
      werk: werkregels,
      matAdvies: t.matAdv,
      matVerkoop: t.matVerk,
      werkTotaal: t.werkTot,
      subtotaal: t.sub,
      btwBedrag: t.btw,
      totaal: t.totaal,
      btwPct: btwTarief,
  ...
  let velWacht = null;
  function tekenVel() {
    /* Een toetsaanslag raakt zowel de som als het veld zelf; zonder deze wachtrij zou het vel
       daardoor twee keer per aanslag opnieuw opgebouwd worden. */
    if (velWacht) return;
    velWacht = requestAnimationFrame(() => {
      velWacht = null;
      const doos = $('of-vel');
      if (!doos) return;
      const doc = documentNu();
      /* Hoeveel bladen het worden vraagt hij aan de PDF-bouwer, want die legt de breuk echt.
         Loopt de offerte over de bladgrens, dan hoort hij dat te zien voor hij hem verstuurt. */
      const paginas = PDF.telPaginas(doc);
      doos.innerHTML = velHtml(doc);
      pasSchaal(doos, { paginas });
      const telling = $('of-vel-paginas');
      if (telling) telling.textContent = paginas > 1 ? `Hij beslaat ${paginas} pagina's.` : '';
    });
  }
  ...
  function tekenWerk() {
    $('of-werkregels').innerHTML = werk.map((w, i) => `
      <div class="fr-veldrij fr-werkregel" data-i="${i}">
        ...
        <button class="fr-knop klein tweede" data-w-weg="${i}" type="button" title="Regel verwijderen">Weg</button>
      </div>`).join('');
    tekenTotalen();
  }
  ...
  function schil() {
    ...
    return `
    <div class="fr-werkblad">
      <div class="fr-invullen">
    <div class="fr-form fr-form-breed">
      ...
      ${regels.length ? `<label>Korting voor je klant op de adviesprijs: <b id="of-korting-label">${korting}%</b>
        ...
        ${/* Myron, testronde 20-09-2026: bij het schuiven zie je niet wat het kost of wat je
              overhoudt, want dat staat pas onder de werkregels. Nu staat het direct onder de
              schuif, waar je het antwoord zoekt op de vraag die je aan het stellen bent. */ ''}
        <p class="fr-calc-verlies" id="of-kortinguit"></p>` : ''}
      ...
      <div class="fr-meekijk">
        <p class="fr-meekijk-kop">Zo ziet hij eruit</p>
        <div class="v-doos" id="of-vel"></div>
        <p class="fr-hint">Dit is het vel dat je klant krijgt. Alles wat je hierboven invult staat er meteen op. <span id="of-vel-paginas"></span></p>
      </div>
    </div>`;
  }
  ...
  async function toon(bak, invoer) {
    ...
    projecten = p ?? [];
    partner = mij.partner;
    logoBeeld = await PDF.logo(partner.logo_data);
    ...
    tekenVrijMat();
    tekenWerk();
    /* De klantvelden, de klus en het aantal dagen raken de som niet, maar wel het vel. Een
       luisteraar op de hele bak vangt ze allemaal, ook velden die er later bij komen. */
    bak.addEventListener('input', tekenVel);
    bak.addEventListener('change', tekenVel);
    volgVenster();
