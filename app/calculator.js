...
export function maakCalculator({ motor, euro, schoon, meldRegel, naBewaren, naOfferte }) {
  let PRODUCTS = [];
  let PARTNER = false;
  /* De partnerprijs per code als bedrag, per stuk inclusief btw, zoals de kassa hem rekent (sinds
     12-09-2026). Staat hij er, dan wint hij van het percentage. */
  let PP = {};
  /* De variant in de winkel per code, voor de winkelwagen bij Oaklyn. */
  let VID = {};
  const OAKLYN = (window.FRAMR_CONFIG && window.FRAMR_CONFIG.OAKLYN_URL) || 'https://oaklyn.nl';
  let PLINTLENGTE = 2.4;
  let LIJM = null;
  const PRIMERS = {};
  let ONDER = null;
  ...
  /* De sleutel blijft framr, want die staat in elke bewaarde berekening en offerte; alleen wat de
     vakman leest verandert. Myron wees erop in zijn testronde (20-09-2026): hij bestelt de vloer
     bij Oaklyn en niet bij Framr.One, dus daar hoort de naam van de winkel te staan. Framr.One is
     het portaal waarin hij werkt, Oaklyn de winkel waar het vandaan komt. */
  const BRONINFO = {
    framr: 'Je bestelt de vloer bij Oaklyn, op jouw partnerprijs. De verzendkosten komen er in de webshop bij en staan niet in deze berekening.',
  ...
  const BRONLABEL = { framr: 'via Oaklyn', extern: 'eigen leverancier', klant: 'levert de klant', onbekend: 'nog te kiezen' };
  ...
  /* Wat er overblijft als je hele verpakkingen bestelt.

     Jelle, 20-09-2026: hij wil dat erbij staat dat het aantal zakken of emmers volgt uit de
     vierkante meters, de laagdikte en het verbruik, en dat het daardoor vaker iets meer is dan
     hij werkelijk nodig heeft. Dat verschil is geen afrondingsfout maar de winkel: je koopt geen
     halve zak. Wie het niet ziet, offreert het ook niet door en betaalt het zelf.

     Onder de vijftig gram is het geen mededeling maar ruis, dus dan blijft het weg. */
  const restRegel = (aantal, inhoud, nodigKg, eenheid) => {
    const over = aantal * inhoud - nodigKg;
    if (!(over > 0.05)) return '';
    const netjes = over >= 10 ? Math.round(over) : Math.round(over * 10) / 10;
    return `, je houdt ${String(netjes).replace('.', ',')} ${eenheid} over`;
  };
  ...
  function consP(code, adv, aantal) {
    ...
    /* Het bedrag van de kassa wint: per stuk inclusief btw, hier terug naar exclusief. */
    if (PP[code] !== undefined && PP[code] !== null) return { p: (PP[code] / 1.21) * aantal, c: advies * 0.8, a: advies };
    const pct = KORT[code];
  ...
  function vulPrijzen(d) {
    ...
    (d.toebehoren || []).forEach((t) => {
      if (t.pp !== undefined && t.pp !== null) PP[t.code] = t.pp;
      if (t.vid) VID[t.code] = t.vid;
    ...
    <div class="fr-calc">
      <div class="fr-calc-in">
        ...
        <div class="fr-form">
          <label>Waar komt de vloer vandaan
            <div class="fr-seg" id="ca-bron">
              <button type="button" data-bron="framr" class="aan">Via Oaklyn</button>
        ...
        <div class="fr-form" id="ca-accs">
          ...
          <div class="fr-acc" id="acc-primer"><label class="kop"><input type="checkbox" id="ca-primer-on" checked> Primer <span class="advies">bij lijm</span>
              <label class="fr-acc-klant"><input type="checkbox" id="ca-klant-primer"> klant levert</label></label>
            <select id="ca-primer-sel"></select>
            <span class="fr-hint">Het aantal volgt uit je vierkante meters en het verbruik van deze primer.
              Je bestelt hele verpakkingen, dus wat je overhoudt staat bij de regel.</span></div>
          ...
          <div class="fr-acc" id="acc-egaline"><label class="kop"><input type="checkbox" id="ca-eg-on" checked> Egaline
            ...
            <span class="fr-hint">Het aantal zakken volgt uit je vierkante meters, de laagdikte en het verbruik van deze
              egaline. Een millimeter dikker scheelt meteen zakken. Je bestelt hele zakken, dus wat je overhoudt staat bij
              de regel.</span></div>
      ...
      <div class="fr-calc-uit">
        ...
        <div class="fr-form" style="margin-top:18px">
          ...
          <div class="fr-knoppenrij">
            ...
            <button class="fr-knop tweede" id="ca-mandje" type="button">Bestellen bij Oaklyn</button>
          </div>
          <p class="fr-hint" id="ca-mandje-uitleg">Bestellen bij Oaklyn zet deze materiaallijst in de winkelwagen van de webshop; daar log je in en reken je af op je partnerprijs. De verzendkosten komen er in die kassa bij en staan niet in deze berekening, dus reken ze mee voordat je offreert. Losse artikelen vind je in <a href="${OAKLYN}/collections/all" target="_blank" rel="noopener">de webshop</a>.</p>
  ...
  function calc() {
    ...
    if (bron === 'framr') {
      ...
      let vInk = vAdv;
      if (p.pp !== undefined && p.pp !== null) {
        /* De partnerprijs zoals de kassa hem rekent: per pak inclusief btw, hier terug naar
           exclusief. Zo komt Jij betaalt op de cent uit op de kassabon van Oaklyn. */
        vInk = pak * (p.pp / 1.21);
      ...
      addRow('Vloer', `${pak} pakken`, `${bruto.toFixed(2)} m² ÷ ${p.pak} m²/pak → ${pak} pak (${(pak * p.pak).toFixed(2)} m²)`, vInk, pak, vVerk, vAdv,
        { code: `vloer:${p.art}`, vid: p.vid || null, bron: 'framr', prijsbasis: basisUitLijst(p.art, vloerLabel, pak ? vInk / pak : null, pak ? vVerk / pak : null, p.adv * p.pak) });
    ...
    if (isDry && $('ca-lijm-on').checked && LIJM) {
      ...
      if (klantLevert('ca-klant-lijm')) {
        ...
        addRow(lijm.naam || `Lijm ${LIJM[0]}`, `${em} ${lijm.eenheid || 'stuk'}`,
          `${opp} m² × ${LIJM[3]} kg = ${lkg.toFixed(1)} kg ÷ ${LIJM[2]} kg/${lijm.eenheid || 'stuk'} → ${em}${restRegel(em, LIJM[2], lkg, 'kg')}`,
      ...
      if (PR && PR[2] && PR[3]) {
        ...
        if (klantLevert('ca-klant-primer')) {
          ...
          addRow(rij?.naam || `Primer ${codePr}`, `${pp} ${rij?.eenheid || 'stuk'}`,
            `${opp} m² × ${PR[3]} kg = ${pkg.toFixed(1)} kg ÷ ${PR[2]} kg → ${pp}${restRegel(pp, PR[2], pkg, 'kg')}`,
      ...
      if (e && e[2] && e[3]) {
        ...
        if (klantLevert('ca-klant-eg')) {
          ...
          addRow(rij?.naam || `Egaline ${codeEg}`, `${zak} ${rij?.eenheid || 'zak'}`,
            `${opp} m² × ${mm} mm × ${e[3]} kg = ${egkg.toFixed(0)} kg ÷ ${e[2]} kg/${rij?.eenheid || 'zak'} → ${zak}${restRegel(zak, e[2], egkg, 'kg')}`,
  ...
  async function toon(bak, voorkeur) {
    ...
    /* De winkelwagen van Oaklyn vullen met wat er via Framr.One besteld wordt: elke regel met een
       variant en een aantal, als winkelwagenlink. De kassa rekent daar de partnerprijs. Wat niet
       in de winkel staat (de vloer van een eigen leverancier, wat de klant levert) blijft hier. */
    $('ca-mandje').addEventListener('click', () => {
      const foutvak = $('ca-fout');
      foutvak.textContent = '';
      const gedaan = VLOEREN.filter((v) => perVloer[v.sleutel]?.uitkomst);
      const lijnen = gedaan.flatMap((v) => perVloer[v.sleutel].uitkomst.lijnen ?? []);
      if (!lijnen.length) { foutvak.textContent = 'Er staat nog niets in je berekening.'; return; }
      const mee = [];
      const niet = [];
      lijnen.forEach((r) => {
        if (r.bron !== 'framr' || !(r.stuks > 0)) return;
        const vid = r.vid || (r.code ? VID[String(r.code).replace(/^[a-z]+:/, '')] : null);
        const nummer = vid ? String(vid).replace(/^gid:\/\/shopify\/ProductVariant\//, '') : '';
        if (/^\d+$/.test(nummer)) mee.push(`${nummer}:${Math.max(1, Math.round(r.stuks))}`);
        else niet.push(r.naam);
      });
      if (!mee.length) { foutvak.textContent = 'Geen van deze regels staat als product in de webshop van Oaklyn.'; return; }
      $('ca-mandje-uitleg').textContent = niet.length
        ? `De winkelwagen is gevuld. Niet in de winkel gevonden en dus niet meegegaan: ${niet.join(', ')}.`
        : 'De winkelwagen van Oaklyn is gevuld met deze materiaallijst; log daar in en reken af op je partnerprijs.';
      window.open(`${OAKLYN}/cart/${mee.join(',')}`, '_blank', 'noopener');
    });
