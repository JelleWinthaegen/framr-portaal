...
export function maakInmeetkaart({ motor, schoon, meldRegel, naBewaren }) {
  ...
  /* Sinds 17-09-2026 draait de kaart om, op verzoek van Jelle: de twee getallen die tellen zijn
     hoeveel vierkante meter vloer erin gaat en hoeveel meter plint. Lengte en breedte zijn een
     hulpmiddel om daaraan te komen en staan achter de knop Uitrekenen. Zijn punt: een maat in je
     hoofd of op je telefoon uitrekenen gaat sneller dan een bijzondere woning proberen te vangen
     in een rechthoek.

     Daarom leven de maten nu hier en niet meer in de invoervelden zelf: het paneel is vaak dicht,
     en dan zou er niets te lezen zijn. matenVan houdt per ruimte lengte, breedte, aftrek en de
     meters zonder plint vast; plintHandVan de met de hand gezette plintmeters (de tegenhanger van
     handmatigVan voor het oppervlak, migration 0114).

     De regel tussen die twee: het laatste wat hij aanraakt wint. Tikt hij een getal in de rij, dan
     is dat het getal. Verandert hij daarna iets in het rekenpaneel, dan volgt de rij de maten
     weer. */
  const matenVan = new Map();
  const plintHandVan = new Map();
  const handGeraakt = new Set();

  const lege = () => ({ l: '', b: '', aftrek: '', plintEraf: '' });
  const matenBak = (rk) => {
    if (!matenVan.has(rk)) matenVan.set(rk, lege());
    return matenVan.get(rk);
  };
  ...
  /* De maten van een ruimte als getal. Komt uit matenVan en niet uit de velden, want die staan in
     een paneel dat meestal dicht is. */
  const maten = (rk) => {
    const m = matenBak(rk);
    return { l: num(m.l), b: num(m.b), aftrek: num(m.aftrek), plintEraf: num(m.plintEraf) };
  };
  ...
  /* De plintmeters, dezelfde keten als de motor (plintMeters in inmeting.ts): met de hand gezet
     wint, dan de wanden, dan de rechthoekregel. Leeg is niet gezet; nul is de keuze dat er geen
     plint komt, en die hoort te blijven staan. */
  function plintVan(rk, m) {
    const wanden = wandenVan.get(rk) ?? [];
    const plintAan = resolveer('plint.toepassen', 'ruimte', rk).waarde !== false;
    const hand = plintHandVan.get(rk);
    if (hand !== undefined && hand !== null && String(hand).trim() !== '') {
      const wandM = wanden.length
        ? wanden.reduce((s, w) => s + Math.max(0, num(w.lengte)), 0)
        : 2 * (m.l + m.b);
      return { m: Math.max(0, num(hand)), wand: wandM, bron: 'handmatig' };
    }
    if (wanden.length) {
  ...
  function geoKnop(soort, rk) {
    ...
    /* De knop gaat over de vlakken zelf. Dat een oppervlak met de hand gezet is staat sinds
       17-09-2026 gewoon in het veld in de rij, dus dat hoeft deze knop niet meer te melden. */
    else if (soort === 'vlakken') tekst = n ? `Vlakken (${n})` : 'Niet rechthoekig';
  ...
  function tekenGeo(soort, rk) {
    ...
    if (!blok) return;
    blok.querySelector(`[data-afwijkplek="${soort}"]`).innerHTML = soort === 'reken' ? rekenKnop(rk) : geoKnop(soort, rk);
    blok.querySelector(`[data-paneel="${soort}"]`).innerHTML = soort === 'reken'
      ? rekenPaneel(rk)
      : (soort === 'vlakken' ? vlakkenPaneel(rk) : wandenPaneel(rk));
    vloerSom($('im-form'));
  }
  ...
  function openGeo(soort, rk) {
    ...
    open.add(sleutel);
    const m = maten(rk);
    if (soort === 'vlakken' && !(vlakkenVan.get(rk) ?? []).length) {
  ...
  /* De knop naar het rekenpaneel. Hij zegt wat hij doet en, zodra er maten staan, dat ze er zijn:
     zo is van buiten te zien waar het getal in de rij vandaan komt. */
  function rekenKnop(rk) {
    const isOpen = open.has(paneelSleutel('reken', rk));
    const m = maten(rk);
    /* De maten alleen tonen als ze ook echt het getal in de rij bepalen. Staat er een met de hand
       ingevuld getal, of komt het uit vlakken, dan zou "Uitgerekend (5,20 bij 4,00)" een getal
       aanwijzen dat nergens meer op het scherm staat. */
    const telt = nettoVan(rk, m).bron === 'rechthoek' && m.l > 0 && m.b > 0;
    const tekst = isOpen ? 'Paneel dicht' : (telt ? `Uitgerekend (${komma(m.l)} bij ${komma(m.b)})` : 'Uitrekenen');
    return `<button type="button" class="fr-knop klein tweede" data-reken="${rk}">${tekst}</button>`;
  }

  /* Het rekenpaneel: de kaart zoals hij was, lengte maal breedte met wat eraf moet. Wat hier
     uitkomt gaat rechtstreeks naar de twee velden in de rij; een aparte overneem-knop zou een
     stap zijn die niets toevoegt, want wie dit paneel opent wil juist dat het uitgerekend wordt. */
  function rekenPaneel(rk) {
    if (!open.has(paneelSleutel('reken', rk))) return '';
    const m = matenBak(rk);
    const attrs = `data-k="${rk}"`;
    return `<div class="fr-afwijk fr-geo">
      <p class="fr-hint">Vul de maten in, dan rekent het portaal de vierkante meters en de plintmeters uit en zet ze hierboven.
        Weet je de meters al, dan hoef je hier niet te zijn: tik ze gewoon in de twee velden.</p>
      <div class="fr-veldrij">
        <label style="max-width:120px">Lengte (m)<input data-maat="l" inputmode="decimal" ${attrs} value="${schoon(m.l ?? '')}"></label>
        <label style="max-width:120px">Breedte (m)<input data-maat="b" inputmode="decimal" ${attrs} value="${schoon(m.b ?? '')}"></label>
        <label style="max-width:120px">Aftrek (m²)<input data-maat="aftrek" inputmode="decimal" ${attrs} value="${schoon(m.aftrek ?? '')}" placeholder="0"></label>
        <label style="max-width:140px">Geen plint (m)<input data-maat="plintEraf" inputmode="decimal" ${attrs} value="${schoon(m.plintEraf ?? '')}" placeholder="0"></label>
      </div>
      <p class="fr-hint" data-rekensom></p>
    </div>`;
  }
  ...
  function ruimteBlok(r = {}, n = 1, key = null) {
    ...
    return `<fieldset class="fr-ruimte" data-key="${rk}" ${r.id ? `data-id="${r.id}"` : ''} ${r.vloer_handle ? `data-handle="${schoon(r.vloer_handle)}"` : ''}>
      ...
      <div class="fr-veldrij">
        ...
        <label style="max-width:150px">Vloer (m²)<input name="r-m2" data-rijmaat="m2" data-k="${rk}" inputmode="decimal" value="" placeholder="0"></label>
        <label style="max-width:150px">Plint (m)<input name="r-plintm" data-rijmaat="plint" data-k="${rk}" inputmode="decimal" value="" placeholder="0"></label>
      ...
      <div class="fr-ruimtevoet">
        ...
        <span class="fr-knoppenrij" style="margin:0">
          <span data-afwijkplek="reken">${rekenKnop(rk)}</span>
      ...
      <div data-paneel="reken">${rekenPaneel(rk)}</div>
  ...
  /* De maten, de vlakken en de wanden van een bestaande ruimte, zoals de motor ze meegeeft. De
     maten staan sinds 17-09-2026 in een paneel, dus ze moeten hier in de bak landen; stonden ze
     nog in de velden, dan waren ze weg zodra het paneel dicht is. */
  function laadGeometrie(r) {
    if (!r.id) return;
    matenVan.set(r.id, {
      l: r.lengte ?? '', b: r.breedte ?? '',
      aftrek: r.aftrek ?? '', plintEraf: r.plint_eraf ?? '',
    });
    if (r.plint_handmatig !== null && r.plint_handmatig !== undefined) plintHandVan.set(r.id, String(r.plint_handmatig));
  ...
  function schil() {
    ...
    <form id="im-form" class="fr-form fr-form-breed" data-id="${inmeting?.id ?? ''}">
      ...
      <div class="fr-bewaarbalk">
        <span class="som" id="im-balksom"></span>
        <span class="knoppen"><span id="im-terugplek"></span><button class="fr-knop" type="submit">Bewaren</button></span>
      </div>
    </form>`;
  }
  ...
  function vloerSom(form) {
    ...
    form.querySelectorAll('#im-ruimtes .fr-ruimte').forEach((blok) => {
      ...
      const bronM2 = netto.bron === 'vlakken' ? ` uit ${tel((vlakkenVan.get(rk) ?? []).length, 'vlak', 'vlakken')}` : '';
      const bronPlint = plint.bron === 'wanden' ? ` uit ${tel((wandenVan.get(rk) ?? []).length, 'wand', 'wanden')}` : '';

      /* De twee velden in de rij volgen de uitkomst, behalve terwijl hij er zelf in typt: dan zou
         het veld zijn eigen invoer overschrijven. */
      const veldM2 = blok.querySelector('[data-rijmaat="m2"]');
      const veldPlint = blok.querySelector('[data-rijmaat="plint"]');
      if (veldM2 && document.activeElement !== veldM2) veldM2.value = netto.m2 ? komma(netto.m2) : '';
      if (veldPlint && document.activeElement !== veldPlint) veldPlint.value = plint.m ? komma(plint.m) : '';

      /* De regel eronder zegt alleen nog waar het getal vandaan komt als dat niet vanzelf spreekt;
         de getallen zelf staan nu in de velden erboven. */
      const bronnen = [
        netto.bron === 'vlakken' ? `vloer${bronM2}` : '',
        netto.bron === 'handmatig' ? 'vloer zelf ingevuld' : '',
        plint.bron === 'wanden' ? `plint${bronPlint}` : '',
        plint.bron === 'handmatig' ? 'plint zelf ingevuld' : '',
        netto.bron === 'rechthoek' && plint.bron === 'rechthoek' && m.l && m.b
          ? `uitgerekend uit ${komma(m.l)} bij ${komma(m.b)}` : '',
      ].filter(Boolean);
      blok.querySelector('.fr-ruimtesom').textContent = bronnen.length ? bronnen.join(', ') : '';
      ...
      const somReken = blok.querySelector('[data-rekensom]');
      if (somReken) {
        somReken.textContent = m.l && m.b
          ? `${komma(m.l)} bij ${komma(m.b)} is ${komma(Math.max(0, m.l * m.b - m.aftrek))} m2 vloer en ${komma(Math.max(0, 2 * (m.l + m.b) - m.plintEraf))} m plint.`
          : 'Vul lengte en breedte in.';
      }
    });
    $('im-vloersom').textContent = `${komma(m2)} m2 vloer, ${komma(omtrek)} m plint`;
    balk.vloer = `${komma(m2)} m² vloer · ${komma(omtrek)} m plint`;
    tekenBalk();
    return { m2, omtrek };
  }

  /* De bewaarbalk onderaan draagt de maat van de opname mee, zodat je onder het scrollen door
     ziet waar je staat: de vloer, de kozijnen, of allebei. */
  const balk = { vloer: '', kozijnen: '' };
  function tekenBalk() {
    const el = $('im-balksom');
    if (!el) return;
    el.textContent = [meet.vloer ? balk.vloer : '', meet.kozijnen ? balk.kozijnen : ''].filter(Boolean).join('   ');
  }
  ...
  function kozijnSom(form) {
    ...
    balk.kozijnen = `${stuks} kozijn${stuks === 1 ? '' : 'en'} · ${oppervlak.toFixed(2).replace('.', ',')} m²`;
    tekenBalk();
  }
    ...
    const ruimtes = meet.vloer
      ? [...form.querySelectorAll('#im-ruimtes .fr-ruimte')].map((blok) => {
          ...
          return {
            ...
            /* De maten komen uit de bak en niet uit velden: die staan in het rekenpaneel en dat is
               meestal dicht. */
            lengte: matenBak(rk).l,
            breedte: matenBak(rk).b,
            aftrek: matenBak(rk).aftrek,
            plint_eraf: matenBak(rk).plintEraf,
            ...
            ...(vlakkenGeraakt.has(rk) || (!blok.dataset.id && (vlakkenVan.get(rk) ?? []).length)
              ? { vlakken: vlakkenVan.get(rk) ?? [] } : {}),
            /* De twee met de hand gezette getallen gaan mee zodra hij aan de meters van deze
               ruimte heeft gezeten, ook zonder vlakken of wanden: sinds 17-09-2026 is dat de
               gewone manier van invullen en niet meer de uitzondering. Null wist hem weer. */
            ...(handGeraakt.has(rk) || vlakkenGeraakt.has(rk)
              ? {
                oppervlak_handmatig: handmatigVan.get(rk) || null,
                plint_handmatig: plintHandVan.get(rk) || null,
              } : {}),
  ...
  async function toon(bak, id, terugHtml, mee = {}) {
    ...
    /* De naam van het werk is de titel van het blad, en tikt live mee. */
    const titel = document.querySelector('.fr-paginakop h1');
    const zetTitel = () => { if (titel) titel.textContent = form.naam.value.trim() || (inmeting ? 'Inmeting' : 'Nieuwe inmeting'); };
    zetTitel();
    form.naam.addEventListener('input', zetTitel);
    tekenVast();
    tekenChecklist();
    tekenEigenWerk();
    ...
    form.addEventListener('click', (e) => {
      ...
      const reken = e.target.closest('[data-reken]');
      if (reken) { openGeo('reken', reken.dataset.reken); return; }
      ...
      if (weg) {
        ...
        matenVan.delete(weg.dataset.ruimteWeg);
        plintHandVan.delete(weg.dataset.ruimteWeg);
        handGeraakt.delete(weg.dataset.ruimteWeg);
        hernummer();
        vloerSom(form);
      }
    });
    ...
    form.addEventListener('input', (e) => {
      ...
      if (vlakVeld) {
        ...
        /* Wie vlakken invult wil dat die gelden, net als bij het rekenpaneel. Zonder dit zou een
           eerder met de hand ingevuld oppervlak blijven winnen en leek het paneel stuk. */
        handmatigVan.delete(rij.dataset.k);
        vlakkenGeraakt.add(rij.dataset.k);
        handGeraakt.add(rij.dataset.k);
        vloerSom(form);
        return;
      }
      /* De twee velden in de rij: wat hij hier intikt is het getal. Leeg maken laat de ruimte weer
         terugvallen op wat de maten of de vlakken zeggen, want een leeg veld is geen nul. */
      const rijmaat = e.target.closest('[data-rijmaat]');
      if (rijmaat) {
        const rk = rijmaat.dataset.k;
        const ruw = rijmaat.value.trim();
        const bak = rijmaat.dataset.rijmaat === 'm2' ? handmatigVan : plintHandVan;
        if (ruw === '') bak.delete(rk); else bak.set(rk, ruw);
        handGeraakt.add(rk);
        vloerSom(form);
        return;
      }

      /* Het rekenpaneel. Wie hier iets verandert wil dat het uitgerekend wordt, dus het met de
         hand gezette getal maakt plaats: anders zou hij maten invullen en niets zien gebeuren. */
      const maatVeld = e.target.closest('[data-maat]');
      if (maatVeld) {
        const rk = maatVeld.dataset.k;
        matenBak(rk)[maatVeld.dataset.maat] = maatVeld.value;
        handmatigVan.delete(rk);
        plintHandVan.delete(rk);
        handGeraakt.add(rk);
        vloerSom(form);
        return;
      }
      ...
      if (wandVeld) {
        ...
        plintHandVan.delete(rij.dataset.k);
      ...
      if (openingVeld) {
        ...
        plintHandVan.delete(wand.dataset.k);
        wandenGeraakt.add(wand.dataset.k);
        handGeraakt.add(wand.dataset.k);
        vloerSom(form);
        return;
      }
